...
def test_get_health_history(client):
    ...
    resp = client.get("/api/v1/health/history/m2?limit=3")
    assert resp.status_code == 200
...
def test_list_machines(client):
    ...
    resp = client.get("/api/v1/health/machines")
    assert resp.status_code == 200
    machines = resp.json()
...
def test_health_not_found(client):
    resp = client.get("/api/v1/health/latest/nonexistent")
...
def test_get_latest_network(client):
    ...
    resp = client.get("/api/v1/network/latest/c1")
    assert resp.status_code == 200
...
def test_get_network_history(client):
    ...
    resp = client.get("/api/v1/network/history/c2?limit=2")
    assert resp.status_code == 200
...
def test_list_controllers(client):
    ...
    resp = client.get("/api/v1/network/controllers")
    assert resp.status_code == 200
    assert any(c["controller_id"] == "ctrl-main" for c in resp.json())


def test_network_not_found(client):
    resp = client.get("/api/v1/network/latest/nonexistent")
    assert resp.status_code == 404
